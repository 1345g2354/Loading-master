        ...
        for (int i = 0; i < mLeafInfos.size(); i++) {
            ...
            if (currentTime > leaf.startTime && leaf.startTime != 0) {
                ...
                int rotate = leaf.rotateDirection == 0 ? angle + leaf.rotateAngle : -angle
                        + leaf.rotateAngle;
                //每一步都是从初始角度直接旋转指定的角度，这个角度根据时间算得,不是逐步累积
...
//        else if (intervalTime > mLeafFloatTime) {//周期性，新一轮
...
//        }
        ...
        /**
         * (当前时间 - 开始时间) = 已经走过的时间
         * (当前时间 - 开始时间)/周期 * 总长度 = 已经走过的长度
         */

        float fraction = (float) (intervalTime % mLeafFloatTime)/ mLeafFloatTime;
    ...
    private int getLocationY(Leaf leaf) {
        // y = A sin(wx+Q)+h    A振幅,2Л/w周期,w/2Л频率,Φ初相  h在y轴上的位移
